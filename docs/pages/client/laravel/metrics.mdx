...
The OTel auto-instrumentation captures default request, DB, queue, and outbound-HTTP metrics automatically (e.g., `http.server.request.duration`, `http.client.request.duration`, `db.client.operation.duration`). To track custom application metrics, use the `Meter` facade.

## The `Meter` Facade

`keepsuit/laravel-opentelemetry` ships a `Meter` facade that gives you direct access to all OTel instrument types. Instruments are cached by name, so calling `Meter::counter('orders.created')` twice returns the same counter — safe to call from anywhere.
...
When several observable instruments share an expensive data source, use `batchObserve` so the source is only consulted once per export cycle:
