...
Because Session Replay runs on [rrweb](https://github.com/rrweb-io/rrweb) under the hood, you can drop rrweb's standard class hooks onto any element in your app to keep sensitive content out of recordings. No SDK configuration needed — these classes are honored automatically.
...
You don't need to add any classes to get the basics — these are always masked:

- `<input type="password">` — value is never captured
- Elements with the `data-sensitive` attribute
- Elements with the `aria-masked` attribute

Reach for the `rr-*` classes whenever the sensitive content isn't in a form field, or when you need stricter masking than the defaults.
