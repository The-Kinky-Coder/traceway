...
- Any exception thrown inside `Tracer::newSpan(...)->measure(fn)` is captured by `measure()` (see [Spans](/client/laravel/spans) for one caveat about span status).
...
When you catch an error but don't rethrow, record it manually:
...
The package's built-in `reportable()` callback runs **before** any callback you register, but you can register your own to add request-level attributes to the span before the exception event is finalized. Use this when you want every error to carry the same diagnostic fields:
...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withExceptions(function (Exceptions $exceptions) {
        $exceptions->report(function (\Throwable $e) {
            // The package already calls recordException + setStatus(ERROR) on
            // the active span — here we just attach extra attributes.
            $span = Span::getCurrent();
