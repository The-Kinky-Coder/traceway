...
```typescript
import { withTraceContext, captureCurrentTrace } from "@tracewayapp/backend";
...
| `distributedTraceId` | `string` | No | Distributed trace ID from an incoming `traceway-trace-id` header (for frontend ↔ backend correlation) |
...
```typescript
import { getTraceContext } from "@tracewayapp/backend";
...
```typescript
import { getTraceId } from "@tracewayapp/backend";
...
```typescript
import { hasTraceContext } from "@tracewayapp/backend";
...
```typescript
import { setTraceAttribute } from "@tracewayapp/backend";
...
```typescript
import { setTraceAttributes } from "@tracewayapp/backend";
...
```typescript
import { setTraceResponseInfo, withTraceContext, captureCurrentTrace } from "@tracewayapp/backend";
...
```typescript
import { captureCurrentTrace, withTraceContext, setTraceResponseInfo } from "@tracewayapp/backend";
...
```typescript
import { captureTrace } from "@tracewayapp/backend";
...
```typescript
import { forkTraceContext, withTraceContext, startSpan, endSpan } from "@tracewayapp/backend";
...
```typescript
import { getTraceDuration } from "@tracewayapp/backend";
...
Get the distributed trace ID from the current trace context:

```typescript
import { getDistributedTraceId } from "@tracewayapp/backend";

const distributedId = getDistributedTraceId();
// Returns undefined if no distributed trace ID is set
```

### Express Middleware with Distributed Tracing

When building a custom Express middleware, read the `traceway-trace-id` header and pass it into the trace context to enable frontend ↔ backend correlation:
...
## Express Middleware Example (Basic)
